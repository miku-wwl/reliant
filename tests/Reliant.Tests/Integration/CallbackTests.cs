...
public class CallbackTests : IClassFixture<PostgreSqlFixture>
{
    ...
    [Fact]
    public async Task DuplicateCallback_ShouldReturn200_AndApplyStateOnlyOnce()
    {
        var (orgId, contributionId) = await SeedDataAsync(ContributionState.Processing);
        var sp = BuildServices();
        ...
        var sender = scope.ServiceProvider.GetRequiredService<MediatR.ISender>();
        var cmd = new HandleProviderCallbackCommand(Callback("evt-dup-1", "succeeded", providerReference: "ref_dup_001"));

        var first = await sender.Send(cmd);
        var second = await sender.Send(cmd);

        Assert.Equal(200, first.StatusCode);
        Assert.Equal(200, second.StatusCode);

        var transitions = await db.Set<StateTransition>().IgnoreQueryFilters()
            .Where(t => t.ContributionId == contributionId && t.ToState == ContributionState.Succeeded)
            .ToListAsync();

        // The Processing -> Succeeded transition must exist exactly once.
        Assert.Single(transitions);

        var inboxes = await db.Set<InboxMessage>().IgnoreQueryFilters()
            .Where(x => x.MessageId == "callback-evt-dup-1")
            .ToListAsync();
    ...
    [Fact]
    public async Task ConcurrentDuplicateCallbacks_ShouldApplyStateOnlyOnce()
    {
        var (orgId, contributionId) = await SeedDataAsync(ContributionState.Processing);
        var sp = BuildServices();
        ...
        var tenantContext1 = scope1.ServiceProvider.GetRequiredService<ITenantContext>();
        tenantContext1.SetTenant(orgId, null, null, "test");
        TenantFilterAccessor.SetOrganizationId(orgId);

        var seedDb = scope1.ServiceProvider.GetRequiredService<ReliantDbContext>();
        seedDb.Set<ProviderReference>().Add(new ProviderReference
        {
            Id = Guid.NewGuid(),
            ContributionId = contributionId,
            OrganizationId = orgId,
            Reference = "ref_concurrent_001",
            ProviderName = "sandbox"
        });
        ...
        var cmd = new HandleProviderCallbackCommand(Callback("evt-concurrent-1", "succeeded", providerReference: "ref_concurrent_001"));
        ...
        var inboxes = await db.Set<InboxMessage>().IgnoreQueryFilters()
            .Where(x => x.MessageId == "callback-evt-concurrent-1")
            .ToListAsync();
        Assert.Single(inboxes);
